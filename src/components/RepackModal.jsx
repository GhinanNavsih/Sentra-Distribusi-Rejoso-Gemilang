...
export default function RepackModal({ onClose, onSuccess }) {
    ...
    const handleRepack = async (e) => {
        ...
        try {
            ...
            alert("Kemasan ulang gagal: " + error.message);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
                <h2 className="text-xl font-bold text-gray-900 mb-4">Kemasan Ulang (Pecah Massal)</h2>

                {loadingProducts ? (
                    <div className="py-10 text-center">Memuat produk...</div>
                ) : (
                    <form onSubmit={handleRepack} className="space-y-6">
                        {/* Item Selection */}
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Sumber (Massal)</label>
                                ...
                                {fromSku && (
                                    <p className="text-xs text-gray-400 mt-1">Buka satuan ini...</p>
                                )}
                            </div>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Target (Eceran)</label>
                                ...
                                {toSku && (
                                    <p className="text-xs text-gray-400 mt-1">..tambahkan ke sini.</p>
                                )}
                            </div>
                        </div>
                        ...
                        {fromSku && toSku && (
                            <div className="bg-gray-50 p-4 rounded border border-gray-200 space-y-4">
                                ...
                                <div className="bg-blue-50 p-3 rounded text-sm text-blue-800 border border-blue-100 mt-2">
                                    <strong>Hasil:</strong> Kurangi {qtyToOpen} {getSourceProduct()?.bulk_unit_name} dari sumber.
                                    Tambahkan {qtyToOpen * conversionRate} {getTargetProduct()?.base_unit} ke target.
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                            ...
                            <button disabled={submitting || !fromSku || !toSku} type="submit" className="px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm font-bold shadow-sm disabled:opacity-50">
                                {submitting ? 'Memproses...' : 'Konfirmasi Kemasan Ulang'}
                            </button>
                        </div>
                    </form>
                )}
